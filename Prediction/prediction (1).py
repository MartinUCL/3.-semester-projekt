...
import joblib  # Updated import statement for joblib
...
def main():
    ...
    # Combine the data
    combined_hourly_data = combine_hourly_data(yesterday, cloud_cover_data, temperature_data, west_prices, east_prices)

    # Load pre-trained model and scaler
    model = joblib.load('multi_output_model.pkl')
    scaler = joblib.load('scaler.pkl')  # Load the scaler

    # Prepare features for prediction (cloud cover, temperature, hour, day_of_week, month)
    features = []
    for hour in range(24):
    ...
    features_df = pd.DataFrame(features, columns=['cloud_cover', 'temperature', 'hour', 'day_of_week', 'month'])
