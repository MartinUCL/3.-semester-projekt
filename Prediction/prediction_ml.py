...
# Updated X to include wind_speed along with cloud_cover, temperature, and timestamp-related features
X = df[['cloud_cover', 'temperature', 'wind_speed', 'hour', 'day_of_week', 'month']]
...
model = MultiOutputRegressor(RandomForestRegressor(n_estimators=100, max_depth=20))
...
mse_west = root_mean_squared_error(y_test['west_price'], y_pred[:, 0])
mse_east = root_mean_squared_error(y_test['east_price'], y_pred[:, 1])
print(f'West Price Mean Squared Error: {mse_west}')
print(f'East Price Mean Squared Error: {mse_east}')

mses_west = r2_score(y_test['west_price'], y_pred[:, 0])
mses_east = r2_score(y_test['east_price'], y_pred[:, 1])
print(f'West Price r2 score: {mses_west}')
print(f'East Price r2 score: {mses_east}')
